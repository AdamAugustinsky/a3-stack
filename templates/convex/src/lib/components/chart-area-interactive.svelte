<script lang="ts">
	...
	function normalizeActivityData(
		rawData: Array<{ date: Date; completed: number; inProgress: number; total: number }>,
		daysToShow: number
	) {
...
{#snippet ActivityChart()}
	{@const filteredData = normalizeActivityData(recentActivity, RANGE_TO_DAYS[timeRange])}
