<script lang="ts">
	...
	async function handleSubmit(event: SubmitEvent) {
		...
		try {
			...
			if (result.error) {
				signUpError = result.error.message ?? 'Failed to create account';
				return;
			}
			...
			signUpError = error instanceof Error ? error.message : 'An unexpected error occurred. Please try again.';
		} finally {
			isLoading = false;
		}
	}
